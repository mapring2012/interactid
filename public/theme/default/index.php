...
$(document).ready(function() {		
	...
	var href = $('#nav li a').each(function(){
        ...
        $('#load').remove();
		$('.wrapper').append('<center><span id="load" style="text-align:center;background:yellow;padding:10px; color:red;font-size:18px;;display:block;width:200px;-webkit-border-radius: 10px;border-radius: 10px;border:1px solid #fff;-webkit-box-shadow: 2px 2px 2px 2px rgba(0, 0, 0, );box-shadow: 2px 2px 2px 2px rgba(0, 0, 0, ); ">LOADING...<br/><img src="<?php echo $this->baseUrl();?>/images/loading.gif" /></span></center>');
		$('#load').fadeIn(5000);
        
		window.location.hash = $(this).attr('href').substr(0,$(this).attr('href').length);
		function loadContent() {
			$('#contentdata').load(toLoad,'');
            $('#contentdata').show('fast');
            $('#load').fadeOut(6000);
		}		
		return false;
     ...
     <!--end error Inform-->    					
        <?php
			if(!isset($content))
				...
				include("../application/views/scripts/".$content);
...
(function (c) {
	...
	c.extend(c.validator, {
		...
		messages : {
			required : "<?php echo $this->translate('requiredfile')?>",
			remote : "Please fix this field.",
			email : "<?php echo $this->translate('validemail')?>",
			url : "<?php echo $this->translate('validurl')?>",
			date : "<?php echo $this->translate('validdate')?>",
			dateISO : "<?php echo $this->translate('validdateISO')?>",
			number : "<?php echo $this->translate('validnumber')?>",
			digits : "<?php echo $this->translate('onlydigit')?>",
			creditcard : "<?php echo $this->translate('creditcardnumber')?>",
			equalTo : "<?php echo $this->translate('samevalue')?>",
			accept : "<?php echo $this->translate('validextension')?>",
			maxlength : c.validator.format("<?php echo $this->translate('morecharacters')?>"),
			minlength : c.validator.format("<?php echo $this->translate('leastcharacter')?>"),
			rangelength : c.validator.format("<?php echo $this->translate('betweenandlongcharacter')?>"),
			range : c.validator.format("<?php echo $this->translate('valuebetween0and1')?>"),
			max : c.validator.format("<?php echo $this->translate('valuelessthanorequal')?>"),
			min : c.validator.format("<?php echo $this->translate('valuegreater')?>")
		},
